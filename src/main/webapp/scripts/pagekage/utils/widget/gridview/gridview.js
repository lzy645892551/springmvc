    ...
    _GridView.prototype.AddEditColumn = function () {
            ...
            for (var i = 0, _count = _editColumn.length; i < _count; i++) {
                ...
                if (!!!_data.visible || !!!_visible) {
                    if (_data.append && _data.append != '') {
                        ...
                        _actionHTML += '<span class="gridView_edit"><a idx="' + i + '" type="' + _this.ReplaceData(_data.type, _sourceData) + '" action="' + _this.ReplaceData(_data.action, _sourceData) + '" ' + _saveAppend + '>' + _data.append + '</a></span>';
                    } else {
                        _actionHTML += '<span class="gridView_edit"><a idx="' + i + '" type="' + _this.ReplaceData(_data.type, _sourceData) + '" action="' + _this.ReplaceData(_data.action, _sourceData) + '">' + (_editName != '' ? _editName : _this.ReplaceData(_data.name, _sourceData)) + '</a></span>';
                    }
                }
        ...
        _$grid.children('tbody').find('a[type!=""]').bind('click', function () {
            ...
            if (typeof _clickCallback == 'function') {
                _clickCallback.call(_$this, _actionType, _actionURL);
                return;
            }
