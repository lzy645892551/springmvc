...
<form action="../room/roomManager.do" method="post">
	<div class="text-c"> 
		<input type="text" class="input-text" style="width:100px" placeholder="项目名称" id="projectName" name="project.projectName">&nbsp;
		<input type="text" class="input-text" style="width:100px" placeholder="房间名称" id="roomCode" name="roomCode">&nbsp;
		<input type="text" class="input-text" style="width:100px" placeholder="用途" id="purpose" name="purpose.purpose">&nbsp;
		<input type="text" class="input-text" style="width:100px" placeholder="户型名称" id="apartmentLayout" name="layout.apartmentLayout">&nbsp;
		<input type="text" class="input-text" style="width:100px" placeholder="单元名称" id="unitCode" name="unit.unitCode">&nbsp;
		<input type="text" class="input-text" style="width:100px" placeholder="楼栋名称" id="buildName" name="build.buildName">&nbsp;
		<input type="text" class="input-text" style="width:100px" placeholder="修改人" id="updateUser" name="updateUser">&nbsp;
		<button type="submit" class="btn btn-success radius" id="" name=""><i class="Hui-iconfont"></i> 搜房间信息</button>
	</div>
</form>
    
	<div class="cl pd-5 bg-1 bk-gray mt-20"> 
	<span class="l"><a class="btn btn-primary radius" onClick="system_category_add('添加房间信息','../room/roomReady.do')" href="javascript:;"><i class="Hui-iconfont"></i> 添加房间基本信息</a></span>
	<span class="l">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a class="btn btn-primary radius" onClick="deleteSelect()" href="javascript:;"><i class="Hui-iconfont"></i> 删除选中房间</a></span>
	<span class="l">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a class="btn btn-primary radius"  href="../room/roomDeleteAll.do"><i class="Hui-iconfont"></i> 删除全部房间</a></span>
	<span class="l">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a class="btn btn-primary radius"  href="../build/roomCreateBatch.do"><i class="Hui-iconfont"></i> 批量生成房间</a></span>
	</div>
...
<!-- 					<th class="sorting" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-label="最终用户: 升序排列" style="width: 280px;">项目编号</th> -->
					<th class="sorting" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-label="最终用户: 升序排列" style="width: 300px;">楼层</th>
					<th class="sorting" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-label="最终用户: 升序排列" style="width: 300px;">房间状态</th>
					<th class="sorting_disabled" rowspan="1" colspan="1" aria-label="采购方" style="width:180px;">户型名称</th>
					<th class="sorting_disabled" rowspan="1" colspan="1" aria-label="采购方" style="width:180px;">楼栋名称</th>
					<th class="sorting_disabled" rowspan="1" colspan="1" aria-label="采购方" style="width:180px;">项目名称</th>
					<th class="sorting" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-label="最终用户: 升序排列" style="width: 300px;">单元号</th>
					<th class="sorting" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-label="最终用户: 升序排列" style="width: 300px;">用途名称</th>
...
<!-- 					<th class="sorting" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-label="备件名称: 升序排列" style="width: 300px;">修改时间</th> -->
					...
					<th class="sorting" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-label="合同额: 升序排列" style="width: 150px;">详情</th>
				  </tr>
			</thead>
          <tbody>          
          		<c:forEach items="${roomList}" var="room" varStatus="stat">
          				<tr class="text-c odd" role="row">
						<td><input type = "checkbox" name ="checkbox" id = "${room.roomId}" onchange="selectPorjectId()" /></td>
						<td class="sorting_1">${stat.index+1}</td>
						<td>${room.roomCode}</td>
						<td>${room.floor }</td>
					    <td>${room.state }</td>
						<td>${room.layout.apartmentLayout }</td>
						<td>${room.build.buildName }</td>
						<td>${room.project.projectName}</td>
						<td>${room.unit.unitCode}</td>
						<td>${room.purpose.purpose}</td>
						<td class="f-14">
						  <a title="编辑" href="javascript:;" onClick="system_category_add('编辑房间信息','../room/roomReady.do?roomId=${room.roomId}')" style="text-decoration:none"><i class="Hui-iconfont"></i></a>
						  <a title="删除" href="javascript:;" onClick="deleteSelect('${room.roomId}')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont"></i></a>
			  		    </td>
			  		    <td>
			  		     <a href = "../room/roomManagerDetail.do?roomId=${room.roomId}" class="btn btn-primary radius" >进入</a>
			  		    </td>
			    	</tr>
          		</c:forEach>     
		    </tbody>
