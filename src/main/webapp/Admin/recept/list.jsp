...
<body>
	...
	<nav class="breadcrumb">
		<i class="Hui-iconfont"></i> 首页 <span class="c-gray en">&gt;</span>
		 销售管理 <span class="c-gray en">&gt;</span>接待管理
	</nav>
			...
			<div  > 
				...
				<div id="DataTables_Table_0_wrapper"
					...
					<div class="dataTables_length" id="DataTables_Table_0_length">
						...
						<a style="float: right;" class="btn btn-primary radius" onClick="system_category_add('添加接待记录','toadd.do')" href="javascript:;">添加</a>
					<table
						...
						<thead>
							<tr class="text-c" role="row">
								...
								<th width="100" class="sorting" tabindex="0"
									aria-controls="DataTables_Table_0" rowspan="1" colspan="1"
									aria-label="规格型号: 升序排列" style="width: 80px;">业务员</th>
								<th width="40" class="sorting_desc" tabindex="0"
									aria-controls="DataTables_Table_0" rowspan="1" colspan="1"
									aria-sort="descending" aria-label="备件名称: 升序排列"
									style="width: 200px;">接待时间</th>
								<th width="80" class="sorting" tabindex="0"
									aria-controls="DataTables_Table_0" rowspan="1" colspan="1"
									aria-label="单位: 升序排列" style="width: 80px;">接待方式</th>
								<th width="80" class="sorting_disabled" rowspan="1" colspan="1"
									aria-label="销售单价" style="width: 80px;">客户姓名</th>
								<th width="80" class="sorting" tabindex="0"
									aria-controls="DataTables_Table_0" rowspan="1" colspan="1"
									aria-label="数量: 升序排列" style="width: 80px;">接待信息</th>
								<th width="80" class="sorting" tabindex="0"
									aria-controls="DataTables_Table_0" rowspan="1" colspan="1"
						...
						<tbody>
							<c:forEach var="recept" items="${list}" varStatus="i">
								<tr class="text-c odd" role="row">
									<td><input type="checkbox" name="mcheck"  value="${recept.receptionRecordId }" id="${recept.receptionRecordId }"></td>
									<td name="count"  id="">${i.count }</td>
									<td name="projectId" id="${recept.receptionRecordId }projectName">${recept.userName }</td>
									<td name="updateTime" id="${recept.receptionRecordId }updateTime" ><fmt:formatDate value="${recept.receptionTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
									<td name="buildId"  id="${recept.receptionRecordId}buildName">${recept.receptionMode}</td>
									<td name="unitCode" id="${recept.receptionRecordId }unitCode">${recept.customerName }</td>
									<td name="roomCount" id="${recept.receptionRecordId }roomCount">${recept.receptionContent }</td>
									<td class="ybox06td03bcenter"><span class="l">  
									<a class="btn btn-primary radius" onClick="system_category_add('修改接待信息','toupdate.do?receptionRecordId=${recept.receptionRecordId}')" 
									href="javascript:;">查看/修改</a></span> 
									<%-- <a class="btn btn-primary radius" href="unitmanage/delete.do?receptionRecordId=${recept.receptionRecordId}" />删除</td> --%>
									<a class="btn btn-primary radius" 
									href="javascript:if(confirm('确定删除吗?'))location='del.do?receptionRecordId=${recept.receptionRecordId}'">删除</a></td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
