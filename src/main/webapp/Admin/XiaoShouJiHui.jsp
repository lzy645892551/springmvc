...
<body>
	...
	<div class="page-container" id="page-content">
		<span class="l"> <a class="btn btn-primary radius"
			onClick="system_category_add('添加销售机会','../xiaoshou/selxiangmu.do')"
			href="javascript:;">添加销售机会</a></span>
		<button class="btn btn-primary radius" onclick="rengou()">转认购</button>
		<button class="btn btn-primary radius" onclick="qianyue()">转签约</button>
						...
						<thead>
							<tr class="text-c" role="row">
								...
								<th width="40" class="sorting_desc" tabindex="0"
									...
									style="width: 80px;">机会名称</th>
								<th width="100" class="sorting" tabindex="0"
									aria-controls="DataTables_Table_0" rowspan="1" colspan="1"
									aria-label="规格型号: 升序排列" style="width: 100px;">意向描述</th>
								<th width="80" class="sorting" tabindex="0"
									aria-controls="DataTables_Table_0" rowspan="1" colspan="1"
									aria-label="单位: 升序排列" style="width: 200px;">意向房间</th>
								<th width="80" class="sorting_disabled" rowspan="1" colspan="1"
									aria-label="销售单价" style="width: 80px;">客户姓名</th>
								<th width="80" class="sorting" tabindex="0"
									aria-controls="DataTables_Table_0" rowspan="1" colspan="1"
									aria-label="数量: 升序排列" style="width: 80px;">客户电话</th>
								<th width="80" class="sorting" tabindex="0"
									aria-controls="DataTables_Table_0" rowspan="1" colspan="1"
									aria-label="单位: 升序排列" style="width: 80px;">业务员编号</th>
								<th width="80" class="sorting" tabindex="0"
									aria-controls="DataTables_Table_0" rowspan="1" colspan="1"
								...
								<th width="80" class="sorting" tabindex="0"
									aria-controls="DataTables_Table_0" rowspan="1" colspan="1"
									aria-label="备注: 升序排列" style="width: 30px;">操作</th>
							</tr>
						</thead>
						<tbody>
							<c:forEach var="htmx" items="${list}" varStatus="a">
								<tr class="text-c odd" role="row">
									<td><input type="checkbox" name="mcheck"
										value="${htmx.saleChanceId}" id="${htmx.saleChanceId }"></td>
									<td name="saleChanceName"
										id="${htmx.saleChanceId }saleChanceName">${htmx.saleChanceName}</td>
									<td name="intentionDescription"
										id="${htmx.saleChanceId }intentionDescription">${htmx.intentionDescription}</td>
									<td name="roomId" id="${htmx.saleChanceId }roomId">${htmx.roomId}</td>
									<td name="customerName" id="${htmx.saleChanceId }customerName">${htmx.customerName}</td>
									<td name="tel" id="${htmx.saleChanceId }tel">${htmx.tel }</td>
									<td name="number" id="${htmx.saleChanceId }number">${htmx.number }</td>
									<td name="userName" id="${htmx.saleChanceId }userName">${htmx.userName }</td>
									<td name="state" id="${htmx.saleChanceId }state">${htmx.state }</td>
									<td class="ybox06td03bcenter"><span class="l"> <a
											class="btn btn-primary radius"
											onClick="system_category_add('修改订单信息','selbyid.do?xiaoshouId=${htmx.saleChanceId}')"
