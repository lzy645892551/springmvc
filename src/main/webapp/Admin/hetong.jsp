...
<script type="text/javascript">
	function BjdBean(roomId, customerName, contractId, signTime, total, payTypeName, name, contractState) {
		this.roomId = roomId;
	...
	function fn1() {
		...
		for (var i = 0; i < selectarr.length; i++) {
			if (selectarr[i].checked) {
				...
				var signTime = document.getElementById(id + "signTime").innerHTML;
				...
				var contractState = document.getElementById(id + "contractState").innerHTML;
				var bean = new BjdBean(roomId, customerName, contractId, signTime, total, payTypeName, name, contractState)
				arr[j] = bean;
				j++;
			}
		}
...
<body>
	...
	<div class="page-container" id="page-content">
		<span class="l"> <a class="btn btn-primary radius"
			onClick="system_category_add('添加合同','../hetong/selfangjian.do')"
			href="javascript:;">添加合同</a></span>
		<button class="btn btn-primary radius" onclick="del()">删除合同</button>
		<button class="btn btn-primary radius" onclick="zuofei()">作废合同</button>
		<a href="zuofeilist.do" class="btn btn-primary radius">作废合同列表</a>
							...
							<c:forEach var="htmx" items="${list}" varStatus="a">
								<tr class="text-c odd" role="row">
									...
									<td class="ybox06td03bcenter"><span class="l"> <a
											...
											onClick="system_category_add('修改订单信息','selbyid.do?contractId=${htmx.contractId}')"
